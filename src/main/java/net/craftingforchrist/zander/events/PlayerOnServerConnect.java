...
public class PlayerOnServerConnect implements Listener {
    ...
    public void PlayerServerSwitchEvent(ServerSwitchEvent event) {
        ...
        try {
            ...
            updatestatement.setString(1, event.getFrom().getName());
            updatestatement.setString(2, event.getPlayer().getUniqueId().toString());
            updatestatement.executeUpdate();
            plugin.getProxy().getConsole().sendMessage(new TextComponent(ChatColor.translateAlternateColorCodes('&', Variables.developmentprefix + " " + player.getDisplayName() + " switched to " + event.getFrom().getName() + ".")));
            ...
            textChannel.sendMessage("** :twisted_rightwards_arrows: ** | " + player.getDisplayName() + " switched to " + event.getFrom().getName() + ".").queue();
